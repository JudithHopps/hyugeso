  ...
  <section id="find">
    <div class="find__form">
      ...
      <div class="find__info">
        ...
        <button
          class="find__info-btn"
          type="submit"
          @click="cerificationNumberSend()"
        >
          전송
        </button>
      </div>
      <div>
        <button class="find__btn" type="submit" @click="login()">
          아이디 찾기
        </button>
      </div>
...
import type { FindIdData } from '../../../../modules/api/types';
import { ref } from 'vue';
...
let certificationAnswer: Ref<number> = ref(0);
let cerificationNumberSendState: Ref<boolean> = ref(false);
let certificationText: Ref<string> = ref('인증');
async function login() {
  const userInformation: FindIdData = {
    name: userName,
    mobile: userPhoneNumber.value,
  };

  pass = await axi.methods.$callAPI(
    'POST',
